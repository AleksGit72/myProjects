...
#  Программа на базе функции chunked() (принимает на вход список и число, задающее размер чанка (куска), b возвращает список из чанков (кусков) указанной длины, необходимо создать)
