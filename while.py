...
for i in range(a, b + 1):    # Создаем внешний цикл 'i' от a до b включительно
    ...
    for j in range(1, i / 2 + 1):    # Создаем внутренний цикл от 1 до 'i' включительно
        if i % j == 0:    # Задаем условие если 'i' % 'j' == 0
            counter += 1    # Счетчик +1
    if counter == 2: # Теперь на равне с внутренним циклом создаем условие для проверки, (простое число делится на
                     #  единицу и на себя следовательно счетчик должен быть равен 2-м) если счетчик равен 2
